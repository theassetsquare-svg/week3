...
var SUMMARY_POOL = [
  ...
  "$vp0와 $vp1, 두 키워드가 $addr 소재 $name의 핵심이다.",
  "$addr 근처 $region 상권이 발달해 $vp0 즐긴 후 다음 자리 이동도 수월.",
  "$addr에서는 금요일 $vp0 분위기와 토요일 $vp1 분위기 차이가 확연.",
  "$vp0 수준 $addr에서 테이블석 사전 예약이 사실상 필수다.",
  "$vp1 감성 인테리어가 $addr 소재 $name에서 사진 찍기에도 좋다.",
  "$vp0 포함 $addr에서 생일·기념일 파티 예약이 꾸준하다. $h0.",
  "$h0. 경험 없는 $region 방문자에게도 $vp0 덕분에 문턱이 낮다.",
  "$addr 소재 $name은 2인~10인 $vp1 포함 단체까지 좌석 커버 가능.",
  "$region 야간 $addr 인근 택시가 많다. $vp0 즐긴 후 귀가 계획을 세우자.",
  "$vp0 중심 음악이 $addr에서 요일별로 변주되는 게 $name의 특징.",
  "$hours 중 $addr에서 오픈 1시간 뒤 도착이 $vp0 경험에 가장 효율적.",
  "$addr 소재 $name은 $vp0 수준 보안 요원 상주로 안전한 환경이다."
];
...
var INTRO_P1 = [
  ...
  "<p>$region 나이트라이프를 말할 때 빠질 수 없는 이름. $name. $h0. 한번 가면 안다.</p>",
...
var INTRO_P2 = [
  "<p>$cl 선택의 핵심은 분위기와 사운드다. $name은 $vp0와 $vp1 두 가지를 전면에 내세운다. 말보다 체감이 빠른 영역이다.</p>",
  "<p>입장하면 가장 먼저 느끼는 건 음악이다. $vp0 수준의 시스템이 홀 전체를 채운다. 발끝부터 올라오는 저음이 몸을 움직이게 만든다.</p>",
  ...
  "<p>$cl마다 저마다의 무기가 있다. $name의 무기는 $vp0와 $vp1. 그 조합이 만들어내는 밤은 독보적이다.</p>",
  "<p>첫인상을 결정짓는 건 $vp0이고, 재방문을 결정짓는 건 $vp1이다. 둘 다 놓치지 않았다.</p>",
  "<p>$vp0 하나만으로도 갈 이유는 충분하다. 거기에 $vp1과 $vp2까지 더해지면 이야기가 달라진다.</p>",
  "<p>좋은 $cl의 조건을 따지면 결국 $vp0와 $vp1로 귀결된다. $name은 그 두 가지에 집중한다.</p>",
  ...
  "<p>사운드가 플로어를 지배한다. $vp0 레벨의 시스템이 깔리고, $vp1이 그 위에 얹어진다. 꽤 설득력 있는 조합이다.</p>",
...
var INTRO_P3 = [
  ...
  "<p>$as 일대에 자리 잡고 있다. $addr 근처, 도보 5분 이내에 역이 있어서 이동이 편하다.</p>",
  ...
  "<p>$addr. 지도에서 보면 $as 핵심 블록 안이다. 주변에 편의점과 다음 자리 갈 만한 곳이 즐비하다.</p>"
];
...
var INTRO_P4 = [
  ...
  "<p>영업시간 $hours. 도착 타이밍에 따라 완전히 다른 경험을 하게 된다. 밤 10시가 갈림길이다.</p>",
...
var STORY_BODIES_0 = [
  ...
  "$region 소재 $name, 안쪽으로 들어서면 바깥과 전혀 다른 세계. $vp0 조명과 $vp1 사운드와 사람들 에너지가 동시에.",
  "$addr 입구를 지나는 순간 소음이 사라진다. $vp0 기반 사운드가 귓가를 채운다. $region의 $type은 귀가 먼저 반응한다.",
  "$addr 소재 $name에 발을 디디면 $vp0 공기가 달라진다. $vp1과 $region 특유의 긴장, 기대, 진동이 동시에 밀려온다.",
  "$addr 문 너머 통로가 나온다. 짧은 복도지만 $region 일상과 밤 경계가 뚜렷하다. $vp0 분위기의 세계가 열린다.",
  "$region 소재 $name 입장하면 멈춰 둘러보게 된다. $vp0 조명, $vp1 밀도, 음악 볼륨이 $addr에서 동시에 들어온다.",
  "$addr 밖에서 상상 어려운 세계가 $region 소재 $name 안에 펼쳐진다. $vp0 조명은 낮고 $vp1 사운드는 높다.",
  "$region 소재 $name, 처음 들어서면 피부가 먼저 반응한다. $vp0 진동이 $addr 바닥을 타고 올라온다. $type 밤은 발밑부터.",
  "$addr의 문을 여는 순간 일상 무게가 떨어진다. $region 소재 $name 안은 $vp0 분위기 속 다른 시간이 흐른다.",
  "$region $addr 입구를 지나 안쪽 10걸음. $vp0 분위기가 $type 수준 100% 전달된다. $vp1까지 공기가 달라지는 순간."
];

var STORY_TITLES_1 = ["사운드와 무대", "음악이 만드는 분위기", "귀가 먼저 반응하다", "사운드 시스템", "음악의 힘", "소리가 플로어를 채울 때", "DJ와 선곡", "비트가 만든 밤"];
var STORY_BODIES_1 = [
  "$region 소재 $name의 핵심은 사운드다. $vp0 수준 시스템이 $addr 인근 홀 전체를 채운다. $hours 운영 중 DJ 선곡이 바뀌면 열기가 올라간다.",
  "$type에서 음악은 배경이 아니라 주인공. $vp0에 투자한 흔적이 $region 현장에서 선명하다. $vp1 분위기 속 베이스가 바닥을 타고 올라온다.",
  "$addr 위치한 $name, 발걸음 옮길 때마다 사운드 결이 달라진다. $vp0 시스템이 $region $type 홀 구석구석을 커버한다.",
  "$region $type의 DJ 부스가 홀 중앙에 자리잡았다. $vp0 기반 사운드가 $addr 소재 홀에 방사형으로 퍼진다. $vp1도 눈여겨볼 점.",
  "$hours 운영 중 음악은 시간대별로 변한다. 초반 가벼운 비트에서 $region 자정 이후 $vp0 시스템 풀파워로. $type 분위기가 확 달라진다.",
  "$region에서 좋은 $cl의 핵심은 사운드다. $vp0 레벨 시스템이 $addr에서 울리면 $vp1 분위기 속 몸이 먼저 반응한다.",
  "$vp0 장비가 $region 소재 $name의 핵심이다. $addr에서 홀에 맞게 조율된 소리가 나온다. $type 경험자라면 차이를 귀로 안다.",
  "$region $type에서 사운드가 플로어를 지배한다. $vp0 시스템의 저음은 $addr 현장에서 가슴을 울린다. $vp1도 함께 경험할 수 있다.",
  "$hours 운영되는 $name에서 DJ 선곡이 밤의 방향을 결정한다. $vp0 시스템 위 $region 음악은 같은 곡도 다르게 들리게 만든다.",
  "$region $addr 소재 $name에서 음악이 분위기를 조절한다. $vp0 시스템의 저음~고음 밸런스가 $type 경험의 무기다."
];

var STORY_TITLES_2 = ["좌석과 동선", "자리 선택 팁", "내부 구성", "어디에 앉을까", "내부 구조", "좌석 배치의 기술", "동선 파악하기", "자리잡기 전략"];
var STORY_BODIES_2 = [
  "$region 소재 $name은 홀 좌석과 테이블석이 분리돼 있다. $vp1 포함 단체라면 사전 예약 필수. $addr 근처 2~3명은 홀에서 자유롭게.",
  "$type 좌석 구조가 명확하다. $region에서 홀·테이블·VIP 분리. $vp0 분위기 속 인원·예산에 맞게 고르면 된다.",
  "$addr 위치한 $name, 안쪽일수록 $vp0 사운드 밀도가 높다. $region $type에서 대화 원하면 입구 쪽, 음악 원하면 안쪽 홀.",
  "$region 소재 $type 테이블은 벽면 배치, 중앙은 오픈 홀이다. $vp0 분위기 속 $addr에서 움직이기 좋은 구조.",
  "$region $type 특성상 층마다 분위기가 다를 수 있다. $vp0 있는 아래층은 활기, 위층은 $vp1 분위기로 조용한 패턴.",
  "$addr 소재 $name의 VIP석은 분리 영역에서 $vp0 프라이빗하다. $region $type 일반석은 개방형으로 에너지 공유.",
  "$region $type 동선이 꼬이지 않는 구조다. $addr 현장에서 바·홀·화장실 이동이 $vp0 분위기 속 자연스럽게 연결된다.",
  "$region $type에서 소규모면 바 카운터, 4인+면 테이블석. $vp1 포함 단체는 $addr 현장에 미리 전화로 자리 확보.",
  "$addr 소재 $name의 좌석 배치가 여유롭다. $region $cl 수준 $vp0 분위기 속 옆 테이블과 거리가 적당해 대화 가능.",
...
var STORY_BODIES_3 = [
  ...
  "$region에서 밤 9시는 워밍업, 11시 본 게임, 새벽 1시 절정이다. $hours 중 $vp0 경험은 시간 선택이 좌우한다.",
  "$addr 소재 $type, 초반 빈 자리가 여유롭다. $hours 중 11시 넘기면 $region 분위기가 달라진다. $vp0 열기가 시작.",
  "$region $type 피크는 금토 자정~새벽 2시. $hours 운영 중 $vp0 분위기 공략하려면 미리 입장해 $addr에서 자리 잡는 게 핵심.",
  "$region 소재 $type은 주중·주말 분위기 차이가 크다. $vp0 특성상 주중 소규모, 주말 대규모 열기. $addr에서 다른 경험 가능.",
  ...
  "$hours 사이 $region $type은 매 시간 분위기 밀도가 달라진다. $vp0 여유 원하면 일찍, $vp1 열기 원하면 늦게 $addr로."
];
...
var STORY_BODIES_4 = [
  ...
  "$region $type에서 매너 있는 손님을 선호한다. $vp0 분위기 속 웨이터 호출은 눈 맞춤이 깔끔. $addr 현장에서 고함보다 제스처.",
  ...
  "$region $type은 예약 시간보다 30분 늦으면 자리가 풀린다. $hours 운영 중 $addr 방문 시 지각 예상되면 미리 연락.",
  ...
  "$region $type은 $hours 운영 중 영업 종료 30분 전 라스트 오더. $addr 방문 시 $vp0 즐기다 퇴장 시간 놓치지 않도록."
];
...
var FAQ_POOL = [
  ...
  { q: "$addr 소재 드레스코드?", a: "$region 소재 $type 특성상, 슬리퍼·반바지는 제한이다. $vp0 분위기에 맞게 깔끔한 캐주얼 이상이면 된다. $addr 인근 업소 공통 룰." },
  ...
  { q: "연령 제한은?", a: "만 19세 미만 입장 불가, 신분증 필수. $addr 소재 $name $hours 운영 시간 동안 미성년자 동반도 제한된다." },
  ...
  { q: "$addr 근처 다음 자리?", a: "$addr 주변 $region 나이트라이프 상권이 밀집돼 있다. $type 후 도보 거리에 술집·식당이 많아 다음 자리 동선이 편리." },
  ...
  { q: "최소 주문 금액?", a: "$addr 소재 테이블석 특성상 최소 주문이 있을 수 있다. $vp0 서비스 포함 홀 이용은 보통 제한 없다. $addr 현장 확인." },
  ...
  { q: "$addr 소재 음식?", a: "$addr 소재에서 $vp0 분위기에 맞는 간단한 안주류가 구비돼 있다. $addr 소재 $name 특성상 식사보다 술 중심 메뉴." },
  { q: "웨이터 서비스?", a: "$region $addr 소재 $type에서 웨이터 상주, $vp0 기반 주문부터 자리 안내까지. $hours 영업 중 편하게 요청." },
  { q: "주 연령대?", a: "$addr 소재 $name 주 연령대는 20대 후반~40대 초반이 중심이다. $hours 중 시간대·요일에 따라 $vp0 분위기와 연령 분포가 변한다." },
  ...
  { q: "소음 수준?", a: "$addr 소재 특성상 $vp0 기반 음악 볼륨이 있다. $addr 소재 $name 특성상 대화 원하면 테이블석이나 조용한 구역 요청." },
...
var QP_TEMPLATES = [
  ...
  { label: "다음 자리 코스", title: "1차 후 $type 이동", desc: "근처 식당에서 1차 마치고 도보 이동. $hours 운영이니 11시쯤 도착하면 타이밍 적절." },
...
var CONCLUSION_POOL = [
  ...
  "$h0. $name이 증명해온 이야기다. $region의 밤, 한 번쯤은 $name에서 보내볼 가치가 있다.",
  "$region 나이트라이프를 제대로 즐기고 싶다면 $name은 반드시 후보에 넣어야 한다. $h0. 말보다 체험이 빠르다.",
  "결국 밤은 어디서 보내느냐가 전부다. $name, $h0. $region에서라면 여기가 답이다.",
  "$name에서 보낸 밤은 기억에 남는다. $h0. $region의 밤을 이야기할 때, $name이 빠지면 아쉽다.",
  ...
  "밤의 가치를 아는 사람에게 $name은 자연스러운 선택이다. $h0. $region의 밤, $name에서 채워보자.",
...
function generateContent(venue, idx) {
  ...
  // Uniqueness comes from REAL venue data (name/region/type/hours/value differences)
  // + per-venue template variation via pickH(idx,...). No artificial salt glyphs.
